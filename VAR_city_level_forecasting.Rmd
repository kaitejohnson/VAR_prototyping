...
# Quick plot
sampled_draws <- sample(1:max(df$draw), 100)
...
of epiweeks. Luckily, the theres a function for `epiweek` in the
`lubridate` package.
We'll start with the full set of data and then filter. Note, we need to 
produce 2 more days of forecasts to forecast out the full 5 epi weeks
(28 days + )
```{r}
df_forecasts <- dfall |>
  filter(date >= ymd(forecast_date) - days(5)) |>
...
stopifnot( "Not all weeks contain 7 days of data",
           all(df_weekly$n_days_data == 7))
...
df_weekly_quantiled <- df_weekly |> 
  ...
  select(reference_date, location, horizon, target, target_end_date,
         output_type, output_type_id, value)
```
Make a quick plot of the forecasts vs the data up until the forecast date
```{r}
data_weekly <- model_data |> 
  mutate(
  ...
  group_by(epiweek, target_end_date, location, reference_date) |>
  summarize(
    obs_weekly_sum = sum(count)
  ) |>
  filter(target_end_date >= reference_date - weeks(10),
         target_end_date != reference_date) |>
  mutate(location = ifelse(location == "Citywide", "NYC", location))

df_quantiles_wide <- df_weekly_quantiled |>
  filter(output_type_id %in% c(0.5, 0.025, 0.975, 0.25, 0.75)) |>
  tidyr::pivot_wider(id_cols = c("location", "target_end_date"),
                     names_from = "output_type_id")
 
  
ggplot() +
  geom_line(data = data_weekly,
            aes(x= target_end_date, y = obs_weekly_sum)) +
    geom_point(data = data_weekly,
            aes(x= target_end_date, y = obs_weekly_sum)) +
  facet_wrap(~location, scales = "free_y") +
